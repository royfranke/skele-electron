...
export default class GameUtilities {
    ...
    animateWater(layer, scene) {
        var self = this;
        for (var i = 0; i < 47; i++) {
            layer.replaceByIndex(TILES.WATER.BITMAP_A_[0].index[i], TILES.WATER.BITMAP_B_[0].index[i]);
        }
        scene.time.addEvent({
            delay: 250,
            loop: false,
            callback: () => {
                for (var i = 0; i < 47; i++) {
                    layer.replaceByIndex(TILES.WATER.BITMAP_B_[0].index[i], TILES.WATER.BITMAP_A_[0].index[i]);
                }
                scene.time.addEvent({
                    delay: 250,
                    loop: false,
                    callback: () => {
                        self.animateWater(layer, scene);
                    }
                });
            }
        });
    ...
    buildFacadeSection(_x, _y, width, height, material, layer) {
        ...
        if (material == undefined) {
            console.warn('No material defined for facade at ' + _x + ', ' + _y);
            console.warn(material);
            return;
        }
        var position = 'LOWER_LEFT_';
        if (material[position] == undefined) {
            position = 'LOWER_';
        }
        layer.weightedRandomize(material[position], _x, _y, width - 2, 1);

        layer.weightedRandomize(material.LOWER_, _x + 1, _y, width - 2, 1);

        position = 'LOWER_RIGHT_';
        if (material[position] == undefined) {
            position = 'LOWER_';
        }

        layer.weightedRandomize(material[position], _x + width - 1, _y, 1, 1);

        if (height > 2) {
            // go up by the number of mid tiles (height-2)
            _y = _y - (height - 2);
            var position = 'MID_LEFT_';
            if (material[position] == undefined) {
                position = 'MID_';
            }
            layer.weightedRandomize(material[position], _x, _y, 1, height - 2);
            ...
            layer.weightedRandomize(material[position], _x + width - 1, _y, 1, height - 2);
        }
        if (height > 1) {
            _y = _y - 1;
            var position = 'TOP_LEFT_';
            if (material[position] == undefined) {
                position = 'TOP_';
            }
            layer.weightedRandomize(material[position], _x, _y, 1, 1);
            ...
            if (tile_attr.BITMAP) {
                if (!TILES[tile_type].BITMAP_) {
                    if (tile_type == 'ROOF') {
                        ...
                        layer.putTileAt(TILES[tile_type].BITMAP_A_[0].index[mapped_index], _x, _y);
                    }
                }
                else {
    ...
    initializeTiles(layer, scene, edgeLayer = null) {
        var initialized = 0;
