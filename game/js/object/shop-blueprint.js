...
export default class ShopBlueprint {
    ...
    plan() {
        return {
            positions: this.calculateShopPositions(),
            wallsBuilt: this.wallsBuilt
        };
    }

    render(plan) {
        const shopPlan = plan ?? this.plan();
        const positions = shopPlan.positions;

        if (!shopPlan.wallsBuilt) {
        ...
        this.buildHours();
        this.bindHoursFromExistingObjects();
    }
    ...
    rollWallKind() {
        ...
        for (var i = 0; i < this.settings.levels.length; i++) {
            if (this.roll([0,1,2,3,4,5]) > 2) {
                var colors = ['YELLOW_COMMERCIAL','RED_COMMERCIAL', 'YELLOW_COMMERCIAL'];
                wallKind.push(WALLTILES.BRICK[this.roll(colors) + "_"]);
            }
            else {
                if (i == 0) {
                    var colors = [WALLTILES.CEMENT.GREEN_WORN_,WALLTILES.CEMENT.RED_YELLOW_WORN_, WALLTILES.CEMENT.BLUE_WORN_, WALLTILES.STUCCO.YELLOW_WORN_];
    ...
    placeStorefrontSigns(x, y) {
        ...
        if (this.prop.listing.slug == 'PSYCHIC') {
            ...
            neon.sprite.setDepth(this.gates[1].sprite.depth - 1);
    ...
    getObjectsAt(_x, _y) {
        ...
        if (registry == undefined || registry.getObjects == undefined) {
            return [];
        }
        const objects = registry.getObjects(_x, _y);
        return Array.isArray(objects) ? objects : [];
    }

    getObjectBySlugAt(_x, _y, slug) {
        const objects = this.getObjectsAt(_x, _y);
        return objects.find(obj => obj?.info?.slug == slug);
    }

    collectStoreWindowGates(_x, _y, width) {
        let remaining = width;
        let index = 0;
        while (remaining >= 4) {
            const gateTileX = _x + (index * 4);
            const gate = this.getObjectsAt(gateTileX, _y).find(obj => obj?.info?.type == 'ROLLING_GATE');
            if (gate != undefined) {
    ...
    resolveSignFromWorld(x, y) {
        const slug = this.prop?.listing?.slug;

        if (slug == 'PAWN SHOP') {
            return this.getObjectBySlugAt(x, y - 3, 'PAWN_STOREFRONT_SIGN');
        }
        if (slug == 'PSYCHIC') {
            return this.getObjectBySlugAt(x + 1, y - 3, 'PSYCHIC_STOREFRONT_SIGN');
        }
        if (slug == 'LAUNDROMAT') {
            return this.getObjectBySlugAt(x, y - 4, 'COIN_WASH_SIGN');
        }
        if (slug == 'BAR') {
            return this.getObjectBySlugAt(x, y - 3, 'SIGN_POUR_YORICK');
        }
        if (slug == 'LOCKSMITH') {
            return this.getObjectBySlugAt(x, y - 3, 'LOCKSMITH_STOREFRONT_SIGN');
        }
        if (slug == 'BONEDEGA') {
            return this.getObjectBySlugAt(x - 1, y, 'LIT_SIGN_BONEDEGA');
        }
        if (slug == 'BARBER') {
            return this.getObjectBySlugAt(x - 1.5, y, 'BARBER_POLE');
        }

        return undefined;
    }

    rebindRuntimeObjectsFromWorld() {
        ...
        this.gates = [];
        this.front_door = undefined;

        if (this.prop.structure.type == 'STOREFRONT' || this.prop.structure.type == 'TAKEOUT-WINDOW' || this.prop.structure.type == 'CORNER-STORE-LEFT') {
            const frontObjects = this.getObjectsAt(x, y);
            this.front_door = frontObjects.find(obj => obj?.info?.type != 'ROLLING_GATE');
            const doorGate = frontObjects.find(obj => obj?.info?.slug == 'ROLLING_GATE_DOOR' || obj?.info?.type == 'ROLLING_GATE');
            if (doorGate != undefined) {
                this.gates.push(doorGate);
            }
            this.collectStoreWindowGates(x + 2, y, width - 2);
        }

        if (this.prop.structure.type == 'CORNER-STORE-RIGHT') {
            const doorX = x + width - 2;
            const frontObjects = this.getObjectsAt(doorX, y);
            this.front_door = frontObjects.find(obj => obj?.info?.type != 'ROLLING_GATE');
            const doorGate = frontObjects.find(obj => obj?.info?.slug == 'ROLLING_GATE_DOOR' || obj?.info?.type == 'ROLLING_GATE');
            if (doorGate != undefined) {
                this.gates.push(doorGate);
            }
            this.collectStoreWindowGates(x, y, width - 2);
        }

        this.sign = this.resolveSignFromWorld(x, y);
    }

    updateShopHoursState(now, today) {
        if (this.prop?.listing?.schedule == undefined || now == undefined || today == undefined) {
            return;
        }

        let schedule = this.prop.listing.schedule[today.weekday.toLowerCase()];
    ...
    applyShopHoursNow() {
        ...
        const today = this.scene?.manager?.time?.today;
        this.updateShopHoursState(now, today);
    }

    bindHoursFromExistingObjects() {
        this.rebindRuntimeObjectsFromWorld();
        this.setRollingGatesFromHours();
        this.applyShopHoursNow();
    }

    setRollingGatesFromHours() {
        if (this._hoursListenerBound) {
            return;
        }
